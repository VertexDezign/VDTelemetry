...
-- `Plow:getAIInvertMarkersOnTurn`: `rotationMax == rotateLeftToMax` is left. That
...
--   * `rotationMax` is in the join stream (Plow:onReadStream/onWriteStream), so the side is
--     correct on a client from the moment it joins.
--   * `limitToField` is NOT. It is broadcast on change (PlowLimitToFieldEvent) but never
...
function VDT.Plow.collect(object)
  ...
  -- (Plow:onLoad), so a machine that declares neither still maps consistently.
