...
The model needs a convention the writer can interpret, because XML distinguishes `#attr` from
child elements and JSON does not. A small node shape carries that:
...
One generic writer per format, no per-field code. (Alternative considered: typed model + two
hand-written serializers — more code, dead simple, no convention to learn. We lean
convention-driven for the JSON payoff, but this is revisitable.)
...
## Guardrails (this is a behavior-preserving refactor)

- **Do not change the XML output.** Keep `ValueMapper` where it is and have the model hold
  today's presentation values, so emitted XML is byte-identical. Keep `VDTelemetry.XML_VERSION`
  and the XSD unchanged.
- **No automated test covers the mod's Lua output** — `shared:jvmTest` tests the *Kotlin* parser,
  not the mod. So before starting, capture the current `vdTelemetry.xml` for the combine and
  tractor+cultivator cases and **diff the refactored output against it** after each slice. That
  is the safety net.
- Lifecycle (`init`, `loadMap`, settings, `update` timer, `addModEventListener`) stays in
  ...
  to the writer instead of doing the walk itself.
- Keep `sourceFiles` load order correct (dependencies first) as new files are added.
