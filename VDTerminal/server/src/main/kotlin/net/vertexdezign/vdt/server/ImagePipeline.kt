...
object ImagePipeline {
    fun process(data: ByteArray, filename: String, pdaWidth: Int, pdaHeight: Int): Pair<ByteArray, String> {
        ...
        val image: BufferedImage = when (ext) {
            "dds" -> toBufferedImage(Dds.decode(data))
            "png", "jpg", "jpeg" ->
                ImageIO.read(ByteArrayInputStream(data)) ?: error("failed to decode $ext")
            else -> return data to "application/octet-stream"
        }
    ...
    private fun crop(img: BufferedImage, targetWidth: Int, targetHeight: Int): BufferedImage {
        ...
        g.drawImage(
            ...
            left, top, left + extractWidth, top + extractHeight,
            null,
        )
        g.dispose()
        return dst
